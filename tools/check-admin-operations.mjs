...
assert.match(questionBank, /multimedia_pregunta!inner/);
assert.match(questionBank, /multimedia_pregunta\.datos/);
assert.match(api, /getAdminUsers/);
assert.match(api, /getAdminReconciliation/);
assert.match(api, /exportAdminFinance/);
assert.match(api, /params\.set\('hasImage', 'true'\)/);
...
assert.match(admin, /topSources: buildTopSources\(humanPageViews\)/);
assert.match(dashboard, /De dónde llegan las visitas/);
assert.match(dashboard, /overview\.topSources/);
assert.match(dashboard, /overview\.devices/);
assert.match(dashboard, /overview\.funnel/);
assert.match(dashboard, /device: usersDevice/);
assert.match(dashboard, /id="admin-users-device"/);
assert.match(dashboard, /hidden overflow-x-auto fine-scrollbar md:block/);
assert.match(dashboard, /grid gap-3 p-3 md:hidden/);
assert.match(dashboard, /Regresó otro día/);
...
assert.doesNotMatch(questionsPage, /N\.º oficial/);
assert.doesNotMatch(questionsPage, /ID interno/);
assert.match(questionsPage, /Solo con imagen/);
